...
export default function App() {
    ...
    async function bootWorkspace() {
      try {
        ...
        const activeDocumentId =
          storedDocuments.find((document) => document.id === lastActiveId)?.id ||
          storedDocuments[0].id;

        setDocuments(storedDocuments);
        setActiveId(activeDocumentId);
      } catch (error) {
        showToast(error instanceof Error ? error.message : "Unable to open workspace");
      } finally {
  ...
  function scheduleSave() {
    ...
    saveTimerRef.current = window.setTimeout(async () => {
      ...
      try {
        ...
        setSaveStatus("Unsaved");
        showToast(error instanceof Error ? error.message : "Unable to save document");
      }
    }, 700);
  }
  ...
  async function createNewDocument() {
    ...
    setDocuments((currentDocuments) => sortDocuments([document, ...currentDocuments]));
  ...
  async function formatMarkdown() {
    try {
      ...
      showToast(error instanceof Error ? error.message : "Unable to format Markdown");
    } finally {
  ...
  async function importMarkdown(event) {
    ...
    const document = createDocument({
      title: file.name.replace(/\.md$/i, "") || extractTitleFromMarkdown(content),
      content,
    });
  ...
  function insertEditorInline(before, after, placeholder) {
    const inserted = insertInline(editorViewRef.current, before, after, placeholder);
    if (inserted) return;

    handleMarkdownChange(`${markdownRef.current}\n\n${before}${placeholder}${after}\n`);
  }
  ...
  function insertEditorBlock(block) {
    ...
    handleMarkdownChange(`${currentMarkdown}\n\n${block.endsWith("\n") ? block : `${block}\n`}`);
  }
        ...
        <div className="rounded-lg border border-brand-cyan/20 bg-brand-panel p-5 shadow-panel">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-display text-xl font-bold">Opening workspace...</p>
            </div>
          </div>
        </div>
      </main>
  ...
  return (
    <main className="app-shell flex h-screen min-h-0 flex-col overflow-hidden bg-brand-bg text-white">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-brand-panelSoft/82 px-4 py-3 backdrop-blur lg:px-5">
        <div className="flex min-w-0 items-center gap-3">
          <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-brand-cyan/20 bg-brand-bg shadow-cyan">
            <img className="h-8 w-8" src={brandMark} alt="Rizam MD" />
          </span>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-slate-300">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/[0.045] px-2.5 py-2">
            <Clock3 className="h-3.5 w-3.5 text-brand-green" aria-hidden="true" />
            {readingTime} min
          </span>
          <IconButton id="copy-markdown" label="Copy" icon={Copy} onClick={copyMarkdown} />
          <IconButton
